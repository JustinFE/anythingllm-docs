---
title: "MCP on Methodius AI Docker"
description: "How to use and debug MCP tools on Methodius AI Docker"
---

import { Callout } from "nextra/components";

<Callout type="error" emoji="️‼️">
  The use of MCP for Methodius AI Docker is **self-hosting only** and is not available in the Methodius AI Cloud service.
</Callout>

## Things to know about MCP on Methodius AI Docker
...
Methodius AI Docker supports `Tools` loading via MCP Servers. We **do not** support Resources, Prompts, or Sampling.
...
Methodius AI _does not_ automatically start MCP servers when the container starts to prevent any overloading of resources on boot.
...
The majority of commands that are required to run a MCP server are available in the Methodius AI Docker container already.

The base image of Methodius AI Docker is `ubuntu:jammy-20240627.1`, so generic Ubuntu commands will be available as the user running the services inside of the container.
...
The MCP Server configuration file is located in the `plugins/Methodius AI_mcp_servers.json` file in the Methodius AI storage directory.

<Callout type="info" emoji="💡">
  The configuration file is automatically created if you open the "Agent Skills" page in the Methodius AI UI.
</Callout>

The storage directory is defined by the `STORAGE_LOCATION` environment variable when you start the Methodius AI Docker container - [see example](/installation-docker/local-docker)
...
This does not require you to restart the Methodius AI Docker container - the currently running MCPs will be killed and rebooted with whatever changes you made to the configuration file.
...
This does not require you to restart the Methodius AI Docker container - the target MCP Server will be started or stopped immediately.
...
Adding an MCP Server is as simple as adding a new tool to the `mcpServers` object in the `Methodius AI_mcp_servers.json` file in your Methodius AI storage directory.
...
This path will then be using the `STORAGE_LOCATION` directory that you defined when you [started the Methodius AI Docker container](/installation-docker/local-docker). From here you can then write and read files to the host machine.
