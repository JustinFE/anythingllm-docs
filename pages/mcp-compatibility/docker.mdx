---
title: "MCP on Methodius Docker"
description: "How to use and debug MCP tools on Methodius Docker"
---

import { Callout } from "nextra/components";

<Callout type="error" emoji="️‼️">
  The use of MCP for Methodius Docker is **self-hosting only** and is not available in the Methodius Cloud service.
</Callout>

## Things to know about MCP on Methodius Docker
...
Methodius Docker supports `Tools` loading via MCP Servers. We **do not** support Resources, Prompts, or Sampling.
...
Methodius _does not_ automatically start MCP servers when the container starts to prevent any overloading of resources on boot.

Methodius _will_ automatically start MCP servers when you open the "Agent Skills" page in the Methodius UI **or** invoke the `@agent` directive.
...
If you mark a tool as `Methodius.autoStart: false` in your configuration file, it will not be started automatically - you will need to start it manually from the UI.

### Command availability

The majority of commands that are required to run a MCP server are available in the Methodius Docker container already.

The base image of Methodius Docker is `ubuntu:jammy-20240627.1`, so generic Ubuntu commands will be available as the user running the services inside of the container.
...
The MCP Server configuration file is located in the `plugins/Methodius_mcp_servers.json` file in the Methodius storage directory.

<Callout type="info" emoji="💡">
  The configuration file is automatically created if you open the "Agent Skills" page in the Methodius UI.
</Callout>

The storage directory is defined by the `STORAGE_LOCATION` environment variable when you start the Methodius Docker container - [see example](/installation-docker/local-docker)
...
This does not require you to restart the Methodius Docker container - the currently running MCPs will be killed and rebooted with whatever changes you made to the configuration file.
...
This does not require you to restart the Methodius Docker container - the target MCP Server will be started or stopped immediately.
...
Adding an MCP Server is as simple as adding a new tool to the `mcpServers` object in the `Methodius_mcp_servers.json` file in your Methodius storage directory.
...
You can also manually remove an MCP Server by removing the object from the `mcpServers` object in the `Methodius_mcp_servers.json` file and clicking "Refresh" in the "Agent Skills" page on the UI afterwards.
...
<Callout type="info" emoji="💡">
  **Please do not open issues about tool issues on GitHub - we are not the MCP authors or maintainers.** 
  ...
  If you are having issues, you should post on the [MCP Discussion board](https://github.com/orgs/modelcontextprotocol/discussions) - or ask in the Methodius Discord server.
</Callout>
...
This path will then be using the `STORAGE_LOCATION` directory that you defined when you [started the Methodius Docker container](/installation-docker/local-docker). From here you can then write and read files to the host machine.
