...
Methodius Desktop version or running Methodius via Docker.

In general, all Methodius instances just need a valid URL to connect to Ollama running anywhere, however there can be some nuances depending on how you are running Methodius or Ollama - in any case, all that is needed is a reachable URL to connect to Ollama.

The most common issue people run into is trying to use `localhost` or `127.0.0.1` to connect to Ollama running on their local machine when running Methodius via Docker - see the [Troubleshooting (Docker)](#troubleshooting-docker) section for how to fix this.

## General Troubleshooting (Desktop & Docker)

On both the Desktop and Docker versions of Methodius, the Ollama URL is automatically detected _if we can detect it_.
If the Ollama URL is not detected, you will need to manually set the Ollama URL in the Methodius settings.
...
If your Ollama URL is not detected because it is not in the list above, you will need to manually set the Ollama URL in the Methodius settings - which will be shown in the UI for you to modify.
...
If you are running Methodius via Docker and you are trying to connect to Ollama running locally on your machine.

If you are seeing no models loaded in Methodius or getting error responses from Ollama - 100% of the time this is beacause you are using the wrong URL in the connection in Methodius.

### `localhost` and `127.0.0.1` do not work on Docker.

On Docker, `localhost` and `127.0.0.1` are **not valid URLs** for the Docker container Ollama connection in Methodius because both of these refer to the _container_ network and **not the host machine**.
...
If you are running Methodius via Docker and are trying to connect to Ollama running on another machine the underlying principle is the same where the Ollama URL is the IP address of the machine running Ollama.
...
<Callout type="info" emoji="ℹ️">
  ...
  There is no way for Methodius to automatically detect the IP address of the machine running Ollama.
</Callout>


## Methodius Cloud + Local Ollama

You **cannot** connect to Ollama running on your local machine when using Methodius Cloud. This would require you to expose your local machine to the internet long-term via a service like [ngrok](https://ngrok.com/) which is **not recommended** and **not secure**.
