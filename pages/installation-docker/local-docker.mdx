...
description: "Local Docker Installation guide for Methodius"
---
...
<Image
  ...
  width={1920}
  quality={100}
  alt="Methodius Installation Local Docker"
/>

# Get Started with Methodius in Docker

## Pull the latest image

```bash copy showLineNumbers
docker pull mintplexlabs/Methodius:latest
```
...
<Tabs items={['Linux/Mac', 'Windows']} defaultIndex="0">
  ...
  export STORAGE_LOCATION=$HOME/Methodius && \
  ...
  mintplexlabs/Methodius
  ```
 </Tabs.Tab>

 <Tabs.Tab>
  ```powershell copy showLineNumbers
  $env:STORAGE_LOCATION="$HOME\Documents\Methodius"; `
...
- `touch server/storage/Methodius.db` to create empty SQLite DB file.
- `cd docker/`
...
### Use the Midori AI Subsystem to Manage Methodius
...
After setting that up, install the Methodius docker backend to the Midori AI Subsystem.
