...
<Callout type="info" emoji="️💡">
    This is the legacy documentation for how documents in Methodius worked. 
    
    As of Methodius 1.8.5, we have a new way to use documents in chat. Upgrade to the latest version to get the best experience.
...
So to understand why this occurs we first need to clear up some confusion on how RAG (retrieval augmented generation) works inside of Methodius.
...
In Methodius, we do not read your entire filesystem and then report that to the LLM, as it would waste tokens 99% of the time.
...
If this is what you want, you are thinking of agents, which are coming to Methodius soon.

## So how does Methodius work?

Let's think of Methodius as a framework or pipeline.
...
7. The vector database then calculates the "nearest" chunk-vector. Methodius filters any "low-score" text chunks (you can modify this). Each vector has the original text it was derived from attached to it.
...
<Callout type="warning" emoji="️⚠️">
  **IMPORTANT!**
    ...
    However, here is where the embedder model used and other Methodius settings can make the most difference. Read more in the next section.
