---
title: "Configuration"
description: "Other settings, environment variables, and configurations for Methodius"
---
...
**Desktop:**
If you are using Methodius Desktop, do not edit the `.env` file. This guide is only for users who are using Methodius Self-hosted or Docker.

</Callout>

# Configuration of Methodius
...
<Callout type="info" emoji="️💡">
  **Tip:** After you set these environment variables, you will need to restart
  the Methodius service or container for the changes to take effect.
</Callout>
...
This blocks any user, including yourself, from viewing chat history from users using the Methodius chat interface **and** via external embed widgets.
...
<Callout type="error" emoji="️🚨">
  **Important:** You should use an independent API key for using this feature so
  ...
  internally facing Methodius instances that are not exposed to the public
...
This feature is most useful for when you have Methodius as a simple sub-service within a much larger system and you want to leverage existing user authentication flows within that system and want to provide a seamless login experience for your users to your Methodius instance.
...
- You should provision an API key for Methodius so you can create new users as well as issue temporary authentication links for users.
- The user must already exist within Methodius before using this feature. You can create a user via the in-app interface or the API.
...
Once enabled, you can issue a temporary authentication link for a user leveraging the `/v1/users/{id}/issue-auth-token` endpoint via the Methodius API.
You simply need to provide the user ID and the API key you created earlier to generate a temporary authentication token that can be used by the target user to login to Methodius.

```bash copy
curl -X POST "https://your-Methodius-instance.com/v1/users/{id}/issue-auth-token" \
...
Now, the user can visit the provided `loginPath` URL in their browser to be automatically logged in to Methodius!

```text copy
https://your-Methodius-instance.com/sso/simple?token=1234567890
```
...
The user will be redirected to the home page of Methodius after logging in.
...
```text copy
https://your-Methodius-instance.com/sso/simple?token=1234567890&redirectTo=/workspaces/sample-workspace
```
...
## Methodius Hub Agent Skills

<Callout type="error" emoji="️🚨">
  **Important:** Agent skills can enable running untrusted code from untrusted
  sources. By default, Methodius will not allow downloading agent skills from
  the Methodius Hub.
</Callout>

Modification of the `COMMUNITY_HUB_BUNDLE_DOWNLOADS_ENABLED` environment variable allows you to pull in agent skills from the Methodius Hub.
...
- `COMMUNITY_HUB_BUNDLE_DOWNLOADS_ENABLED=1`: This configures enables Methodius to download agent skills from the Methodius Hub but **only if the item is verified or a private item**.
- `COMMUNITY_HUB_BUNDLE_DOWNLOADS_ENABLED=allow_all`: This configures enables Methodius to download agent skills - including unverified public items - from the Methodius Hub.
...
By default, Methodius attempts to use streaming for a better user experience. However, some custom LLM implementations may not support this feature, resulting in errors or unexpected behavior.
...
By default, Methodius will use a TTL of **30 days** for sessions.
