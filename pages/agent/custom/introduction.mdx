...
Custom agent skills are a powerful feature of Methodius, but they can also be dangerous if misused.
...
Methodius allows you to create custom agent skills that can be used to extend the capabilities of your `@agent` invocations. These skills can be anything you want from a simple API call to even operating-system invocations.

The sky is the limit! Depending on how you run Methodius, you can create custom agent skills that can run extra processes like running a local Python script or, on Desktop, even operating-system invocations.

If it can be done in NodeJS, it can likely be done in Methodius.

## The current state of custom agent skills

<Callout type="info" emoji="️💡">
 Custom agent skills are newly supported in Methodius and may have some bugs, quirks, missing features, unsupported features, etc.
...
4. There is currently no established tooling for creating custom agent skills - so follow this guide if developing skills for Methodius.
...
Custom agent skills are available in Methodius Desktop version **1.6.5 and later.**

Custom agent skills are **not** available in the Methodius Cloud offering.

## View loaded custom agent skills

You can view the loaded custom agent skills by opening the `Agent Skills` tab in the settings of Methodius.

Any valid custom agent skills loaded into Methodius will be displayed here.
